...
function bindTreeSwitch() {
  const swLabel = document.querySelector('label.switch[data-field="hasTree"]');
  const sw = swLabel ? swLabel.querySelector('input[type="checkbox"]') : null;
  const panel = document.querySelector('.tree-panel');
  if (!sw) return;
  sw.addEventListener('change', () => {
    set({ hasTree: sw.checked });
    if (panel) panel.hidden = !sw.checked;
  });
}
...
export function syncFormFromStore() {
  ...
  // 树开关
  const treeSwLabel = document.querySelector('label.switch[data-field="hasTree"]');
  const treeSw = treeSwLabel ? treeSwLabel.querySelector('input[type="checkbox"]') : null;
  const treePanel = document.querySelector('.tree-panel');
  if (treeSw) { treeSw.checked = form.hasTree; if (treePanel) treePanel.hidden = !form.hasTree; }
}
